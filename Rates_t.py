...
# Create lag variables for AAA and BBB
data['AAA_lag'] = data['AAA'].shift(1)
data['BBB_lag'] = data['BBB'].shift(1)

# Perform linear regression of Mortgage Rates on AAA, AAA_lag, BBB, and BBB_lag
X = data[['AAA', 'AAA_lag', 'BBB', 'BBB_lag']].dropna()  # Drop rows with NaN values
X = sm.add_constant(X)  # Add a constant term (intercept)
y = data.loc[X.index]['MORTGAGE30US']  # Match indices with X
...
# Print out residuals, mean, and stdev
print(ar_residuals)
print(f"\nMean of AR(1) Residuals: {ar_residual_mean}")
print(f"Standard Deviation of AR(1) Residuals: {ar_residual_std}")
